...
/*   Updated: 2024/04/17 17:25:57 by psalame          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_square(t_mlx *mlx, int x, int y)
{
	...
	while (++y_map < 20 - 1)
	{
		x_calc = (mlx->stg->minimap_pos_x + 155) + ((x - mlx->map->player_pos.x) * 20);
		y_calc = (mlx->stg->minimap_pos_y + 91) + ((y - mlx->map->player_pos.y) * 20);
		x_map = -1;
